...
Transaction.prototype.toBufferWriter = function(writer) {
  ...
  if(this.version > 1) {
    var timestamp = this.timestamp ? this.timestamp :  Math.round(new Date().getTime()/1000);
    this.timestamp = timestamp; // force update of timestamp on object
...
Transaction.prototype.toSigningBuffer = function() {
  var writer = new BufferWriter();
  return this.toSigningBufferWriter(writer).toBuffer();
};

Transaction.prototype.toSigningBufferWriter = function(writer) {
  writer.writeInt32LE(this.version);
  writer.writeVarintNum(this.inputs.length);
  _.each(this.inputs, function(input) {
    input.toBufferWriter(writer);
